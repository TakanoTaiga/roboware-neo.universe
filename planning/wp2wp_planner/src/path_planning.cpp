...
status PathPlanning::global_path_init(
  ...
  const boost_type::polygon_2d_lf & robot, nav_msgs::msg::Path & result_path, rclcpp::Logger logger)
{
  if (check_pose_in_map(pose_current, map, robot) == outside_pose) {
    RCLCPP_ERROR_STREAM(
      logger, "pose_current is outside map. " << pose_current.pose.position.y << " , "
  ...
  if (check_pose_in_map(pose_goal, map, robot) == outside_pose) {
    RCLCPP_ERROR_STREAM(
      logger, "pose_goal is outside map. " << pose_goal.pose.position.y << " , "
                                           << pose_goal.pose.position.y);
    return outside_pose;
  }

  init_path_generator(pose_current, pose_goal, result_path);
...
status PathPlanning::map_avoidance_planner(
  ...
  const boost_type::polygon_2d_lf & robot)
{
  std::vector<std::pair<geometry_msgs::msg::PoseStamped, geometry_msgs::msg::PoseStamped>>
    error_poses;
