...
{
bool GraphUtil::isSpace(unsigned char c) { return std::isspace(c); }
...
std::string GraphUtil::str_graph_tostring(mission_graph_str graph)
{
  ...
  for (const auto & pair : graph) {
    out_show += pair.first + "(" + pair.second.infomation + ")-" + std::to_string(pair.second.id) +
                " connects to: ";
...
std::string GraphUtil::bin_graph_tostring(mission_graph_bin graph)
{
  ...
  for (const auto & pair : graph) {
    out_show +=
      std::to_string(pair.second.id) + "-" + pair.second.strategy_label + " connects to: ";
    for (const auto & conn : pair.second.connections) {
