...
: rclcpp::Node("MissionManagerNode", node_option)
{
  ...
  // Graph visualize
  RCLCPP_INFO_STREAM(
    logger, gen_mission_graph.str_graph_tostring(gen_mission_graph.at_mgraph_str()));
  RCLCPP_INFO_STREAM(
    logger, gen_mission_graph.bin_graph_tostring(gen_mission_graph.at_mgraph_bin()));

  // ROS 2 initialize
  pub_task_action_ = create_publisher<rw_planning_msg::msg::TaskAction>("output/task_action", 0);
  ...
  sub_action_result_ = create_subscription<rw_planning_msg::msg::ActionResult>(
    "input/action_result", 0,
    std::bind(&MissionManagerNode::action_result_subscriber_callback, this, std::placeholders::_1));

  st_timer_ = create_wall_timer(
    std::chrono::milliseconds(1000),
    std::bind(&MissionManagerNode::state_transition_callback, this));
}
...
void MissionManagerNode::state_transition_callback()
{
  ...
  if (state_transition_handler.is_end()) {
    // rclcpp::shutdown();
    const auto _ =
      system("ps aux | grep ros | grep -v grep | awk '{ print \"kill -9\", $2 }' | sh");
  }
}

void MissionManagerNode::action_result_subscriber_callback(
  const rw_planning_msg::msg::ActionResult & action_result)
{
