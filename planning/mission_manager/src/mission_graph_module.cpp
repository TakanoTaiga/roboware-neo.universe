...
void MissionGraph::get_str_graph(const std::string & file_path, mission_graph_str & graph_str)
{
  ...
  while (std::getline(file, line)) {
    ...
    if (
      std::getline(iss, source, '-') && std::getline(iss, arrow, '>') &&
      ...
      std::string result;
      if (get_information(source, result)) graph_str[source_key].infomation = result;
      if (get_information(target, result)) graph_str[target_key].infomation = result;
    }
  }
  // add id
